...
function AboutSpiral({ darkMode, spiralControls, handControls }) {
	const opacityResetRef = useRef(null)
	...
	const spiralVariants = {
		...
		visible: {
			...
			pathLength: 1,
			strokeOpacity: [null, 0.1, 0.025, darkMode ? 0.02 : 0.05],
			transition: {
				type: 'spring',
				duration: 3,
			},
	...
	useEffect(() => {
		if (darkMode) {
			opacityResetRef.current.style.strokeOpacity = '0.02'
		} else {
			opacityResetRef.current.style.strokeOpacity = '0.05'
		}
	}, [darkMode])

	return (
		<motion.svg
			className='absolute top-0 left-[25%] hidden h-full w-full blur-[2px] xs:block'
			width='400'
			height='400'
			viewBox='0 0 400 400'
			fill='none'
		...
		>
			<motion.path
				...
				stroke='#ffacc6'
				strokeOpacity={darkMode ? '0.02' : '0.05'}
				strokeWidth='0'
				strokeLinecap='round'
				initial='initial'
